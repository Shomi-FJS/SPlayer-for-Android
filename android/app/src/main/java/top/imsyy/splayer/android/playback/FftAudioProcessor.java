...
/**
 * ExoPlayer 解码链中的 FFT 处理器，输出 fftBins[256] + lowFreq。
 *
 * <p>lowFreq 算法完全对齐 PC 端 {@code AudioEffectManager.getLowFrequencyVolume()}：
 * <ol>
 *   <li>取 0–280Hz 频带均值（覆盖 sub-bass 与 kick drum 主能量）。</li>
 *   <li>threshold=180 过滤底噪：音乐常态低频约 180–255，阈值隔离起鼓点 attack 的瞬时超出量。</li>
 *   <li>pow(x, 2) 扩展动态范围：弱信号压低、强信号保留。</li>
 *   <li>EMA 平滑系数 0.28，保留 attack 同时压住抖动。</li>
 * </ol>
 *
 * <p>之前的 80–120Hz 窄频带 + peak 方案丢弃原因：该区间恰好夹在 kick fundamental (50–70Hz)
 * 与 punch (100–200Hz) 之间，错过主能量；中频泛音反而通过 Hann 旁瓣泄漏进来造成“钢琴/擦片声
 * 推动背景”的错误现象。
 *
 * <p>fftBins[256] 仍为 80–2000Hz 紧凑频谱，供前端 PlayerSpectrum 可视化。
 *
 * <p>PCM 原样透传下游；除 setListener 外都在音频线程调用。
 */
@UnstableApi
public final class FftAudioProcessor extends BaseAudioProcessor {
  ...
  /**
   * setListener 跨线程重置状态的传递令牌。
   *
   * <p>旧实现从任意线程直写 sampleBufferPos / lastCallbackNs / lowFreqSmoothed（非 volatile），
   * 在音频线程正读写同一组字段时构成 data race。现改为：主线程只置位 volatile flag，
   * 音频线程在 analyze() 入口 “拉令牌” 后自行重置。
   */
  ...
  private void analyze(ByteBuffer inputBuffer) {
    ...
    for (int frame = 0; frame + strideFrames <= totalFrames; frame += strideFrames) {
      ...
      if (sampleBufferPos >= FFT_SIZE) {
        ...
        // 50% overlap：保留后半作下一窗前半
        System.arraycopy(sampleBuffer, HOP_SIZE, sampleBuffer, 0, HOP_SIZE);
        sampleBufferPos = HOP_SIZE;
      }
    }
  }
