...
public final class PlaybackUrlResolver {
  ...
  /** 失败 songId 短期负缓存窗口（毫秒）：跨多次 prefetch 周期同一首失败时避免持续打 NCM /song/url。 */
  private static final long NEGATIVE_CACHE_TTL_MS = 30_000L;

  /** 2 线程：避免「播放刚起 + 用户立刻 NEXT」被串行。上游 NCM API 有节流，不宜调高。 */
  ...
  public synchronized void updateContext(
    ...
    this.apiBaseUrl = baseUrl == null ? "" : baseUrl.trim();
    this.cookie = cookieValue == null ? "" : cookieValue.trim();
    if (level != null && !level.isEmpty() && !level.equals(this.songLevel)) {
      // 音质变化后旧缓存 URL 对应不同文件/码率，全部作废；
      // 同时清负缓存，避免之前底 level 下一过性失败被错误拦截。
      synchronized (cache) {
        cache.clear();
      }
