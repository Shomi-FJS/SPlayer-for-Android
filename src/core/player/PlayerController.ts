...
class PlayerController {
  ...
  public setupSongUI(song: SongType, startSeek: number) {
    ...
    // status-store 用 $patch 批量提交：把 7-8 个独立 setter 合成一次 $subscribe 触发，
    // pinia-plugin-persistedstate 据此只做 1 次 JSON.stringify+localStorage.setItem，
    // 避免切歌瞬间在主线程上连续触发多次持久化写。
  ...
  private buildAndroidWindowTracks(_currentSong: SongType): {
    ...
    // personalFM=0；普通列表 ±20 = 41 首窗口
    const radius = personalFm ? 0 : 20;
    ...
    for (let i = windowStart; i <= windowEnd; i++) {
      ...
      if (!song) continue;
      // 被跳过的歌曲推 url=null，Java 端 advance/back 时会跳过
