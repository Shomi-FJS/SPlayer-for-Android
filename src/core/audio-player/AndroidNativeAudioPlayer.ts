...
export class AndroidNativeAudioPlayer extends EventTarget implements IPlaybackEngine {
  ...
  public async enableVisualizer(enable: boolean): Promise<boolean> {
    ...
    // 串行化：把每次调用 chain 到上一个 op 之后，避免 PlayerSpectrum/PlayerBackground 快速
    // mount→unmount 时 enable(true) 与 enable(false) 并发到达原生侧，最终态与 TS 标记错位。
    const op = this.visualizerOpQueue.then(async () => {
      ...
      try {
        ...
        this.visualizerEnabled = enable && result.granted;
        if (!enable) {
          // 关闭后清零缓存，避免下次开启前还在显示旧的"鼓点"
          this.latestFftData.fill(0);
          this.latestLowFreq = 0;
        }
    ...
    // catch 防止 unhandled rejection 阻断后续 op
  ...
  private async bindNativeListeners() {
    ...
    // visualizerData: Java 端 Visualizer FFT 数据 ~20Hz 推送，缓存到本地以便 rAF 同步读取
    this.listenerHandles.push(
      await AndroidNativePlayback.addListener("visualizerData", (event) => {
        if (Array.isArray(event.fft) && event.fft.length === this.latestFftData.length) {
          // 直接覆盖到既有 Uint8Array 避免每次分配新数组
          for (let i = 0; i < event.fft.length; i++) {
            this.latestFftData[i] = event.fft[i] & 0xff;
          }
        }
